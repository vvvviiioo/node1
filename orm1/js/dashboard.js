...
async function loadUserData() {
    try {
        ...
        document.getElementById('userGreeting').textContent = `Добро пожаловать, ${checkResult.user.username}!`;
        ...
        const isAdmin = checkResult.user.id === 2;
        if (isAdmin) {
...
async function loadLoginHistory() {
    try {
        ...
        if (historyData.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="3" style="text-align: center;">История входов отсутствует</td></tr>';
        } else {
